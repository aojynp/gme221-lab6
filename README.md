...
E.  Final Reflection Questions

11. How is GeoAI different from traditional GIS analysis? 
    Traditional GIS analysis relies heavily on deterministic, rule-based operations and manual spatial queries such as simple buffer operations or overlaying intersecting maps, to find patterns. In contrast, GeoAI integrates machine learning models to automatically learn non-linear relationships, multi-dimensional correlations, and complex patterns directly from geographic datasets. It moves spatial analysis from a descriptive framework to a predictive framework that forecasts attributes or classifications based on implicit spatial characteristics.

12. What spatial features most influenced the model? 
    While a Support Vector Machine (SVM) using a Radial Basis Function (RBF) kernel does not results to a straightforward linear feature importance rankings. The model's accuracy relies heavily on the spatial and contextual metrics engineered from the datasets. In the predictive model in under Challenge Exercise, it calculated via Shannon Entropy within a 1km buffer for land use diversity. This feature gives the model an indicator of the functional mix of the neighborhood like locating highly mixed urban cores from homogeneous residential zones. For tiurism density, the model enables to count tourist attractions within the local neighborhood buffer provides a direct economic and commercial signal, heavily influencing classifications related to commercial or recreational zoning. Lastly for proximity specifically, the road and tourism proximity, the model computed distance values such as distance to road and distance to tourism, act as critical continuous gradients that allow the SVM to establish complex decision boundaries based on accessibility and infrastructure concentration.

13. What are the limitations of this model? 
    While the model is highly capable, it has several limitations that are common in spatial machine learning. First is relying on a fixed, arbitrary 1km buffer radius that assumes the neighborhood influences stop abruptly at a hard boundary rather than fading out gradually. Second, there is a prominent risk of target leakage because the inputs, landuse_code and the target target_code, are tightly linked, which can deceptively inflate training accuracy. Finally, SVM models naturally struggle with highly skewed or sparse count variables such as localized tourism numbers while we scaled our features properly using a pipeline. They demand heavy computational power, meaning the code will slow down drastically if scaled up to larger cities.

14. How can this support spatial decision-making?
    From a planning perspective, this model offers immense value by serving as an intelligent automation tool for land administration. Urban planners can utilize these predictive classifications to conduct rapid zoning audits, automatically flagging parcels where the actual, predicted use conflicts with outdated official city records. Furthermore, by experimenting with variables like tourism density or proximity to infrastructure, decision-makers can simulate "what-if" planning scenarios and determining the strengths, weaknesses, opportunities, and threats. This allows cities to forecast how a newly proposed highway, school, or tourist spot might alter surrounding real estate dynamics and reshape neighboring communities before a single shovel hits the ground.

15. What ethical or planning concerns may arise from predictive parcel classification?
    Deploying predictive algorithms to govern land use brings up several ethical and social concerns. Algorithms manipulate strictly from historical data, any systemic inequities, redlining, or segregated zoning practices embedded in past planning data will simply be internalized and replicated by the SVM model. Furthermore, leaning too heavily on automated predictions risks stripping away human empathy and hyper-local qualitative knowledge, leading to rigid planning decisions that ignore unique community cultures or environmental nuances. Finally, if these predictive outputs fall into the hands of aggressive real estate developers and commercial-biased planners, they can trigger intense property speculation, artificially driving up values and accelerating gentrification that displaces vulnerable, long-term residents.
